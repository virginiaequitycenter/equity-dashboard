# Get ACS5 5-year survey data - Blockgroup Level
# Last updated: 4/9/2026

# This script gets and cleans ACS5 data for a single survey year. 
# The resulting file is saved as `acs5_blockgroup.RDS`. 
...
blkgrp_hisp  <- blkgrp_race %>% 
  filter(variable == "B03002_012") %>% 
  rename(lxE = estimate,
         lxM = moe) %>% 
  select(-variable) %>% 
  left_join(blkgrp_tot) %>% 
  mutate(hispE = round((lxE/estimate)*100, 2),
         hispM = round((moe_prop(lxE, estimate, lxM, moe))*100, 2)) %>% 
...
blkgrp_data <- blkgrp_data %>% 
  ...
  select(GEOID, NAME, year, totalpopE, totalpopM, whiteE, whiteM, blackE, blackM, asianE, asianM, 
         indigE, indigM, othraceE, othraceM, multiE, multiM, hispE, hispM, snapE, snapM, everything())
...
# ....................................................
# Save ----
saveRDS(blkgrp_data, file = paste0(filepath_data, "acs5_blockgroup.RDS")) 

# Remove vars from environment
rm(list = ls(pattern = "^varlist_"))
rm(list = setdiff(ls(pattern = "^blkgrp_"), "blkgrp_data"))
